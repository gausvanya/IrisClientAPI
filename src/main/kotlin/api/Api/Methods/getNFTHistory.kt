...
import IrisClientAPI.Api.Seralizable.NFTHistorySerialization
...
internal suspend fun IrisApiClient.getNFTHistory(offset: Int, limit: Int, method: String): NFTHistorySerialization? {
    return withContext(Dispatchers.IO) {
        try {
            ...
            if (response.status == HttpStatusCode.OK) {
                ...
                json.decodeFromString<NFTHistorySerialization>(jsonResult)
            } else throw IrisResponseException("${response.bodyAsText()} (${response.status.value})")

        } catch (e: IrisResponseException) {
            logger.error { "Ошибка при получение истории обмена NFT $e" }
            null
        }
    }
}
